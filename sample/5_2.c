...
HeadNode* Issingle(HeadNode* Pfind) {
    while (Pfind != nullptr ) {
        if(Pfind->Num == 1)
            return Pfind;
        Pfind = Pfind->down;
...
HeadNode* replicate(HeadNode* LIST) { //此处检验传参正常，开始检查复制有无逻辑错误
...
void Delhead(HeadNode *Clause,HeadNode *&LIST) {
    if (!Clause) return;
    if(Clause == LIST)
        LIST = Clause->down;
    else {
...
status DPLL(HeadNode *LIST,consequence *result) {
    //单子句规则
    HeadNode* Pfind = LIST;
    HeadNode* SingleClause = Issingle(Pfind);
    while (SingleClause != nullptr) {
        ...
        Delhead(SingleClause,LIST);//删除单子句这一行
        DeleteDataNode(temp,LIST);//删除相等或相反数的节点
        if(!LIST) return TRUE;
        else if(Isempty(LIST)) return FALSE;
        Pfind = LIST;
        SingleClause = Issingle(Pfind);//回到头节点继续进行检测是否有单子句
    }
    //分裂策略
    int Var = LIST->right->data;//选取变元
    HeadNode* replica = replicate(LIST);//存放LIST的副本replica
